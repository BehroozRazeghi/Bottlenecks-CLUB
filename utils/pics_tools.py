...
def plot_image_grid(x_data, img_shape, grid_shape):
  ...
  fig, ax = plt.subplots(grid_shape[0], grid_shape[1], dpi=300)
...
def plot_image_grid_with_label(x_data, img_shape, grid_shape, xlabels:list):
  ...
  if one_dim:
    for i in np.arange(max(grid_shape)):
      ...
      ax[i].get_xaxis().set_ticks([])
      ax[i].get_yaxis().set_ticks([])
      ax[i].set_xlabel(xlabels[data_idx])
    ...
    for i in np.arange(grid_shape[0]):
      for j in np.arange(grid_shape[1]):
        ...
        ax[i,j].get_xaxis().set_ticks([])
        ax[i,j].get_yaxis().set_ticks([])
        ax[i,j].set_xlabel(xlabels[data_idx])
